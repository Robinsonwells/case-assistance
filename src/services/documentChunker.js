...
export default class DocumentChunker {
  ...
  chunkHybrid(text, sentenceWindowSize = 8, sentenceOverlap = 2) {
    try {
      ...
      for (let paraIdx = 0; paraIdx < paragraphs.length; paraIdx++) {
        ...
        // Extract sentences from current paragraph (with enhanced validation)
  ...
  _isSentenceBoundary(textSoFar, nextChar, nextNextChar) {
    ...
    // FIXED: Rule 5 - Enhanced check for legal citation patterns in trailing context
    // Look at last 20 chars to detect incomplete citations like "(Admin. R. 181)"
    const trailing = textSoFar.slice(-20);
    
    // Pattern: "(Admin. R." or "(Admin. R. 1" (incomplete citation)
    if (/\(Admin\.\s*R\.?(\s+\d*)?$/.test(trailing)) {
      return false; // NOT a sentence boundary - citation incomplete
    }
    
    // Pattern: "(Admin. R. at" (incomplete citation with "at")
    if (/\(Admin\.\s*R\.\s+at\s*$/.test(trailing)) {
      return false;
    }
    
    // Pattern: Other common legal reporters "F. 3d", "U. S.", etc.
    if (/\b(F|U|P|A|N\.Y|N\.E|S\.W)\.\s*(\d+[a-z]{0,2})?\s*$/.test(trailing)) {
      return false;
    }
    
    // Rule 6: Check if the text ends with a known abbreviation
    ...
    // FIXED: Enhanced title/honorific detection with stricter rules
    const titlePattern = /^(Dr|Mr|Mrs|Ms|Prof|Rev|Hon|Sr|Jr|Esq)$/i;
    if (titlePattern.test(wordWithoutPeriod)) {
      // Conservative: assume it's "Dr. <Name>" and NOT a sentence boundary
      // Only treat as boundary if next char is clearly starting a new sentence
      if (/[A-Z]/.test(nextChar)) {
        // Could be "Dr. Smith" or "Dr. This is new"
        // Play it safe: DON'T break here
        return false;
      }
      return false; // Definitely not a boundary if not followed by uppercase
    }
    ...
    // Rule 7: Single letter followed by period (initial) - NOT a boundary unless followed by uppercase
    // Example: "J. Smith" vs "J. This is a new sentence"
    if (/^[A-Z]$/.test(wordWithoutPeriod)) {
      return /[A-Z]/.test(nextChar)
    }
    
    // Rule 8: Legal citation patterns like "Admin. R." or "U. S." - NOT boundaries
    if (words.length >= 2) {
    ...
    // Rule 9: If followed by uppercase letter or digit, it's likely a sentence boundary
  ...
  _isFragmentParagraph(text) {
    ...
    // FIXED: 4. Check for name list patterns (likely continuation)
    // Pattern: "Holtzmeier, Dr. Seymour, and Dr." or "Smith, Jones"
    if (/^[A-Z][a-z]+,\s+(Dr|Mr|Mrs|Ms)/.test(trimmed)) {
      return true; // Likely a continuation of a list of names
    }
    
    // Pattern: Short paragraphs starting with "Name, Name" (< 200 chars)
    if (/^[A-Z][a-z]+,\s+[A-Z][a-z]+/.test(trimmed) && trimmed.length < 200) {
      return true; // Short paragraphs with comma-separated names are fragments
    }
    
    // 5. Doesn't end with sentence terminator
    // Exception: Allow if it's a proper bullet point or numbered list
    if (!/[.!?"]$/.test(trimmed) && !/^\d+\./.test(trimmed)) {
      return true
    }
    
    // 6. Incomplete citation patterns
    // Example: "(Admin. R." or "(Admin. R. at" or "[his]"
    if (/\(Admin\.\s+R\.\s*$/.test(trimmed) || /\(Admin\.\s+R\.\s+at\s*$/.test(trimmed)) {
      return true
    }
    
    // 7. Weird ellipsis formatting common in legal docs
    // Example: ". . disability . . ." or ". . . from engaging . . ."
    if (/^\.\s+\.\s+/.test(trimmed) || /\.\s+\.\s+\.$/.test(trimmed)) {
      return true
    }
    
    // 8. Ends with incomplete bracket or parenthesis reference
